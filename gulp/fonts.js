...
function fonts(done) {
	 ...
	 *  Font files to the working 'Build' directory
	...
	return g
		...
		.pipe(
			plumber(),
			log(c.bold(c.cyan('Font Development Files Building...')))
		)
		.on('data', function () {
			nSrc += 1;
		})
		.pipe(changed(paths.fnt.dev))
		.pipe(g.dest(paths.fnt.dev))
		.on('data', function () {
			nDes += 1;
		})
		.on('finish', function () {
			log(c.cyan(c.bold('Font Development Files Results...')));
			log(
				c.cyan('Out of'),
